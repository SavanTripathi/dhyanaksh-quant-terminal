# Dhyanaksh HTF Quantitative Terminal — Daily Prospective Paper Collector (Hardened Mode)
...
Write-Output "$(Get-Date -Format 'yyyy-MM-dd HH:mm:ss') [INFO] Invoking Hardened Daily Prospective Paper Collector (--mode prospective)..." | Out-File -FilePath $LogFile -Append

try {
    $env:ENABLE_LIVE_BROKER_EXECUTION = "false"
    python -m scripts.run_daily_prospective_collector --mode prospective
